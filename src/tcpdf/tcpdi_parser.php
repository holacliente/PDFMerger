...
// -----------------------------------------------------------------------------
    ...
    protected function getRawObject($offset=0, $data=null) {
        ...
        // get first char
        $char = isset($data[$offset]) ? $data[$offset] : '';
        // get object type
        switch ($char) {
    ...
    private function getDictValue($offset, &$data) {
        ...
        $offset += 2;
        do {
            if (isset($data[$offset]) && isset($data[$offset + 1]) && $data[$offset] == '>' && $data[$offset + 1] == '>') {
                $i--;
                $dict .= '>>';
                $offset += 2;
            } else if (isset($data[$offset]) && isset($data[$offset + 1]) && $data[$offset] == '<' && $data[$offset + 1] == '<') {
                $i++;
                $dict .= '<<';
                $offset += 2;
            } else {
